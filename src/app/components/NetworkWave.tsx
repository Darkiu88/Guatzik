...
export function NetworkCard({ upload, download }: NetworkCardProps) {
  ...
  const [historyUp, setHistoryUp] = useState<number[]>(new Array(BARS_COUNT).fill(0));
  const [historyDown, setHistoryDown] = useState<number[]>(new Array(BARS_COUNT).fill(0));
  ...
  const getHeight = (val: number) => {
    const maxVal = 100; // Tope visual (100 Mb/s)
    const safeVal = Math.min(val, maxVal);
    return (safeVal / maxVal) * 100; 
  };
